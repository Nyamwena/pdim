...
public class ScanClothes extends AppCompatActivity implements OnTouchListener, CvCameraViewListener2 {
        ...
        public void onManagerConnected(int status) {
            switch (status) {
                case LoaderCallbackInterface.SUCCESS: {
                    ...
                    mOpenCvCameraView.setOnTouchListener(ScanClothes.this);
                }
                break;
                default: {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_scan_clothes);
    ...
    public void loadJSONFromAsset(String colorHex) {
        String json = null;
        try {
            ...
            for(int i= 0; i<jsonArray.length(); i++){
                ...
                if(obj.getString("hex").equals(colorHex.toLowerCase())){
                    color_name.setText(obj.getString("name"));
                    Log.d("Clor",obj.getString("name") );
                } else {
                    Log.d("Invalid color ", colorHex);
                  //  color_name.setText(colorHex);
                }
            }
    ...
    public boolean onTouch(View v, MotionEvent event) {
     ...
     touch_color.setText("#" + String.format("%02X", (int)mBlobColorRgba.val[0])
        ...
        touch_coordinates.setTextColor(Color.rgb((int)mBlobColorRgba.val[0],
                (int)mBlobColorRgba.val[1],
                (int)mBlobColorRgba.val[2]));
        String my_colour = "#" + String.format("%02X", (int)mBlobColorRgba.val[0])
                + String.format("%02X", (int)mBlobColorRgba.val[1])
                + String.format("%02X", (int)mBlobColorRgba.val[2]);
        loadJSONFromAsset(my_colour);
