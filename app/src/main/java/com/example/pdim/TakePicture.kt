...
class TakePicture : AppCompatActivity(), SurfaceHolder.Callback, Camera.PictureCallback {
    ...
    private fun requestPermissions(permissions: Array<String?>) {
        ActivityCompat.requestPermissions(this@TakePicture, permissions, REQUEST_CODE)
    }
            ...
            REQUEST_CODE -> {
                for (result in grantResults) {
                    if (result == PackageManager.PERMISSION_DENIED) {
                        Toast.makeText(this@TakePicture, R.string.permission_warning, Toast.LENGTH_LONG).show()
